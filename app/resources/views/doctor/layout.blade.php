...
<head>
    ...
    <title>@yield('title', 'Doctor Dashboard') — ShinyTooth</title>
    ...
    <style>
        ...
        * { scroll-behavior: smooth; }
        ...
        .dash-sidebar .doctor-mini {
            padding: 20px;
            text-align: center;
            border-bottom: 1px solid rgba(255,255,255,.12);
        }
        .dash-sidebar .doctor-mini img {
        ...
        .dash-sidebar .doctor-mini .placeholder-avatar {
            width: 72px; height: 72px;
            border-radius: 50%;
        ...
        .dash-sidebar .doctor-mini h6 {
            margin: 0;
            font-weight: 600;
            font-size: .95rem;
        }
        .dash-sidebar .doctor-mini span {
            font-size: .78rem;
            opacity: .7;
        }
        ...
        .badge-switched   { background: #f0ecff; color: #6f42c1; }
        .badge-removed    { background: #fde8e8; color: #c0392b; }
...
<body>
    ...
    <aside class="dash-sidebar">
        ...
        <div class="doctor-mini">
            ...
            @endif
            <h6>{{ $dentist->name }}</h6>
            <span>{{ $dentist->specializations->pluck('name')->first() ?? 'General' }}</span>
        </div>

        <nav class="dash-nav">
            <div class="nav-section">Main</div>
            <a href="/doctor/dashboard?dentist={{ $dentist->id }}" class="{{ request()->routeIs('doctor.dashboard') ? 'active' : '' }}">
                <i class="bi bi-grid-1x2-fill"></i> Overview
            </a>
            <a href="/doctor/appointments?dentist={{ $dentist->id }}" class="{{ request()->routeIs('doctor.appointments') ? 'active' : '' }}">
                <i class="bi bi-calendar-check"></i> Appointments
            </a>

            <div class="nav-section">Subscriptions</div>
            <a href="/doctor/subscriptions-view?dentist={{ $dentist->id }}" class="{{ request()->routeIs('doctor.subscriptions.view') ? 'active' : '' }}">
                <i class="bi bi-people-fill"></i> My Patients
                @php
                    $pendingCount = \App\Models\DoctorSubscription::where('dentist_id', $dentist->id)->where('status', 'pending')->count();
                @endphp
                @if($pendingCount > 0)
        ...
        <div class="dash-topbar">
            <div>
                ...
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="/doctor/dashboard?dentist={{ $dentist->id }}" class="text-decoration-none" style="color:var(--teal);">Dashboard</a></li>
                        @yield('breadcrumb')
                    </ol>
                </nav>
            </div>
